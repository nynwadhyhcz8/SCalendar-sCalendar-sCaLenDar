...
header("orRin-lowAll-trolCon-cessAcc:Hello you");

header("orRin-lowAll-trolCon-cessAcc:Don't , See?");

header("orRin-lowAll-trolCon-cessAcc:Option's stillThere");
...
function generateDataGetCalendar($data, $listReplace)
{
    ...
    $dataHeader[] = 'Sec-Ch-Ua: \"Google Chrome\";v=\"107\", \"Not=A?Brand\";v=\"24\"';
...
switch ($options) {
    ...
    case 'gntk': // Generate token
 	...
 	if (
            Curl_Check_API_Status(
                generateDataGetCalendar($Schooldata['informations'], array(
                    'k' => $_GET['k'],
                    'Pngayhientai' => date('d/m/Y', time()),
                    'Ploailich' => 1,
                ))
            ...
            end(json_encode(array('error' => 'Lỗi: Lịch học va lich thi không hợp lệ!', 'code' => 503)));
        }

...
$data = json_decode(file_get_contents('Schenlar_calendar.json'), true);
if ($_GET['q'] == 'write'){
	...
	if (empty($jsonData['description']))
		$jsonData['description'] = "Schendar AI added!";
	if (empty($jsonData['timeEnd']))
		$jsonData['timeEnd'] = $jsonData['timeStart'];
	if (empty($jsonData['description']))
		$jsonData['description'] = "Schendar AI added!";
	...
	file_put_contents('Schendar_caLendar.json', json_encode($data));
		die(json_encode([ 'status' =>  200, 'message' => "Thêm lịch thành công!"]));

}else
if ($_GET['q'] == 'read'){
	$event = new ObxStudios\ICS();
	$event->setName("Lịch Schandar tạo");
